...
for fn in range(len(TRAIN_FILES_0)): # len(TRAIN_FILES_0)
    ...
    x,y,z,u,v,w = provider.loadDataFile(TRAIN_FILES_0[train_file_idxs[fn]]) # 1 x NUM_POINT x 1
...
class SphereAttack():
    ...
    def drop_points(self, pointclouds_pl, vel_pl, labels_pl, sess):## BxNx3
        pointclouds_pl_adv = pointclouds_pl.copy()
        vel_pl_adv = vel_pl.copy()
        for i in range(self.k):
            pl_adv = np.stack([pointclouds_pl_adv, vel_pl_adv],-1) # BxNx3x2
            grad = sess.run(self.grad, feed_dict={self.pointclouds_pl: pl_adv,
            ...
            tmp_vel = np.zeros((vel_pl_adv.shape[0], vel_pl_adv.shape[1]-self.a, 3), dtype=float)
            
            for j in range(pointclouds_pl.shape[0]):
                tmp[j] = np.delete(pointclouds_pl_adv[j], drop_indice[j], axis=0) # along N points to delete
                tmp_vel[j] = np.delete(vel_pl_adv[j], drop_indice[j], axis=0)
            pointclouds_pl_adv = tmp.copy()
            vel_pl_adv = tmp_vel.copy()
            
        return pointclouds_pl_adv,vel_pl_adv
    ...
    def plot_advsarial_samples(self, pointclouds_pl_adv, labels_pl, pred_val):
        ...
        for i in range(labels_pl.shape[0]):
            ...
            if labels_pl[i]!=pred_val[i] and not self.count[labels_pl[i]]:
                ...
                img_filename = 'label_%s_pred_%s.jpg' % (labels_pl[i],
                                                         pred_val[i])
...
def evaluate(num_votes):
    ...
    for batch_idx in range(num_batches):
        ...
        cur_batch_data_adv,vel_batch_data_adv = attack.drop_points(coord_pc[start_idx:end_idx, :, :],
